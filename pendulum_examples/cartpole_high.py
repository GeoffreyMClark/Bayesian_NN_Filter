...
class CartPoleEnvNoise(gym.Env):
    """
    Description:
    ...
    def __init__(self, noise, variance):
        register(id='CartPole-v9', entry_point='pendulum_examples.cartpole_noise:CartPoleEnvNoise')
        ...
        self.action_size = 20
        self.action_space = spaces.Discrete(self.action_size+1) #add the zero condition
    ...
    def step(self, action):
        ...
        action_noise = np.random.normal(0,self.variance)
        new_action = np.clip(action+action_noise, 0, self.action_size).astype(int)
        action = new_action if np.random.uniform(0,1) > self.noise else action

        x, x_dot, theta, theta_dot = self.state
        force = (action-(self.action_size/2))/(self.action_size)*self.force_mag

        # force = -force if np.random.uniform(0,1) >= self.noise else force
